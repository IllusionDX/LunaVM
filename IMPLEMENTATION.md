...
| `0.3.5-alpha` | **Current** | Polymorphic RNG system: single `Random` class serves multiple algorithms via tag in `_state[0]`. `random.PCG(seed)` and `random.Xorshift(seed)` constructors. Algorithm-specific `step` functions with shared dispatch. `random.choice(seq)` for lists/strings. Removed type guards from callable dispatch — dev responsibility or use `random.choice`. |
