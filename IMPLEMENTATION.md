...
| High | **Constant Folding + Peephole** | **B** *Partial* — Compile-time evaluation of `1 + 2` and dead code elimination pending. Peephole: `OP_LT`/`OP_LE`/`OP_GT`/`OP_GE` + `OP_JZ` fused into single opcodes in compiler. | Low — compiler-only, zero runtime risk |
...
| Medium | **Bytecode Peephole (Aggressive)** | **S** *Partial* — `OP_LT`/`OP_LE`/`OP_GT`/`OP_GE` + `OP_JZ` fused (Done). `OP_ADD r0,r0,1` → `OP_INCR r0` pending. | Medium — new opcodes + compiler pattern matching |
