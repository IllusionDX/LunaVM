...
### Future
- Generational GC
- ~~Object finalizers~~ *Done in 0.3.8-alpha* — `OBJ_USERDATA` with tagged finalizers (`new_userdata_tagged`) for file handles, sockets, etc.
...
| Medium | **Superinstructions** | **S** *Done* — Fused `OP_LT`/`OP_LE`/`OP_GT`/`OP_GE` + `OP_JZ` into `OP_LT_JZ`/`OP_LE_JZ`/`OP_GT_JZ`/`OP_GE_JZ`. Added `OP_ADDI_FROM`/`OP_SUBI_FROM` to eliminate `MOVE+ADDI`/`MOVE+SUBI` patterns. Added compare-and-branch with immediate variants (`*_JZ_IMM`) to eliminate `LOADI+compare` patterns. Added `OP_ADDK`/`OP_MULK` to fuse `LOADK+add`/`LOADK+mul` for constant-pool operands. fib30: 0.090s → 0.078s (~13%). bench_entities: 0.120s → 0.117s (~2.5%). | Low — additive opcodes + compiler peephole |
...
| High | **Delete per-instance `member_ic` inline cache** | **S** *Done in 0.4.4-alpha* — Changed `member_ic` indexing to call-site-keyed (`CHUNK ^ IP`). Eliminates thrashing when many instances of the same class access different fields in the same loop. |
| High | **Double Arithmetic Fast-path** | **S** *Done in 0.4.4-alpha* — Direct floating-point math in `OP_ADD/SUB/MUL/ADDK/MULK` handlers. Avoids `do_arith()` dispatch overhead for the common case of double-double operations. |
| Medium | **Leaf Call Fast Path** | **S** *Done* — Functions/methods with no calls, no upvalues, no defaults, no try/catch skip PUSH_FRAME/OP_RET overhead. Saves ~20% on entity update benchmark (0.117s → 0.094s). | Low — per-function is_leaf bit + dispatch check |
