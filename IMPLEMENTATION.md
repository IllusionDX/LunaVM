...
| `0.3.4-alpha` | **Current** | Callable instances: `OP_CALL` now checks for `_call` method on `ObjInstance`. `random.PCG(seed)` instances can be called directly: `rng()` returns a random float, same as `rng.float()`. Consistent with Luna's `_init` naming convention for special methods. |
