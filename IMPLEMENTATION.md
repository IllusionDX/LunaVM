...
| `0.4.1-alpha` | **Current** | ADDK/MULK super instructions (fuse LOADK+arith, ~2.5% on entities, ~13% on fib cumulative). Incremental tri-color GC with write barrier. IMPLEMENTATION.md optimization table consolidated. |
...
| Medium | **Bytecode Peephole (Aggressive)** | **S** *Partial* — `OP_LT`/`OP_LE`/`OP_GT`/`OP_GE` + `OP_JZ` fused (Done). `OP_ADDK`/`OP_MULK` (fuse LOADK+arith, Done). `OP_INCR r0` pending. | Medium — new opcodes + compiler pattern matching |
