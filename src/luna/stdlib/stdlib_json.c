...
static void json_error(JsonParser *p, const char *msg) {
    luna_throw(p->vm, luna_fe(p->vm)->runtime_error_class, "JSON parse error at pos %zu: %s", p->pos, msg);
}
...
static void encode_value(VM *vm, Value v, char **buf, size_t *len, size_t *cap, int depth) {
    if (depth > 100) {
        luna_throw(vm, luna_fe(vm)->runtime_error_class, "json.encode(): nesting too deep");
    }
...
static Value json_parse_native(VM *vm, Value *args, int n) {
    if (n != 1) {
        luna_throw(vm, luna_fe(vm)->argument_error_class, "json.parse() expects exactly 1 argument");
    }
    if (!IS_STRING(args[0])) {
        luna_throw(vm, luna_fe(vm)->type_error_class, "json.parse() argument must be a string");
    }
...
static Value json_encode_native(VM *vm, Value *args, int n) {
    if (n != 1) {
        luna_throw(vm, luna_fe(vm)->argument_error_class, "json.encode() expects exactly 1 argument");
    }
    size_t cap = 256;
    size_t len = 0;
    char *buf = (char*)malloc(cap);
    if (!buf) {
        luna_throw(vm, luna_fe(vm)->runtime_error_class, "json.encode(): out of memory");
    }
