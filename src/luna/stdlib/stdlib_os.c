...
static void require_string(VM *vm, Value v, const char *fn, int idx) {
    if (!IS_STRING(v)) {
        luna_throw(vm, luna_fe(vm)->type_error_class, "%s() argument %d must be a string", fn, idx);
    }
}
...
static Value os_getcwd(VM *vm, Value *args, int n) {
    (void)args;
    if (n != 0) {
        luna_throw(vm, luna_fe(vm)->argument_error_class, "os.getcwd() takes no arguments");
    }
    char buf[4096];
#ifdef _WIN32
    if (!_getcwd(buf, sizeof(buf))) {
        luna_throw(vm, luna_fe(vm)->runtime_error_class, "os.getcwd(): %s", strerror(errno));
    }
#else
...
static Value os_chdir(VM *vm, Value *args, int n) {
    if (n != 1) {
        luna_throw(vm, luna_fe(vm)->argument_error_class, "os.chdir() expects exactly 1 argument");
    }
    require_string(vm, args[0], "os.chdir", 1);
#ifdef _WIN32
    if (_chdir(as_cstring(args[0])) != 0) {
        luna_throw(vm, luna_fe(vm)->runtime_error_class, "os.chdir(): %s", strerror(errno));
    }
#else
...
static Value os_listdir(VM *vm, Value *args, int n) {
    if (n != 1) {
        luna_throw(vm, luna_fe(vm)->argument_error_class, "os.listdir() expects exactly 1 argument");
    }
    ...
    if (!pattern) {
        luna_throw(vm, luna_fe(vm)->runtime_error_class, "os.listdir(): out of memory");
    }
    ...
    free(pattern);
    if (handle == -1) {
        luna_throw(vm, luna_fe(vm)->runtime_error_class, "os.listdir(): %s", strerror(errno));
    }
    do {
...
static Value os_mkdir(VM *vm, Value *args, int n) {
    if (n != 1) {
        luna_throw(vm, luna_fe(vm)->argument_error_class, "os.mkdir() expects exactly 1 argument");
    }
    require_string(vm, args[0], "os.mkdir", 1);
    const char *path = as_cstring(args[0]);
#ifdef _WIN32
    if (_mkdir(path) != 0) {
        luna_throw(vm, luna_fe(vm)->runtime_error_class, "os.mkdir(): %s", strerror(errno));
    }
#else
...
static ObjUserdata *file_userdata_from_instance(VM *vm, Value self, const char *fn) {
    if (!IS_INSTANCE(self) || !AS_OBJ(self)) {
        luna_throw(vm, luna_fe(vm)->type_error_class, "%s() expects a File instance", fn);
    }
    ObjInstance *inst = (ObjInstance*)AS_OBJ(self);
    Value handle = instance_get_field(inst, "_handle");
    if (!IS_USERDATA(handle) || !AS_OBJ(handle)) {
        luna_throw(vm, luna_fe(vm)->runtime_error_class, "%s() called on closed file handle", fn);
    }
    ...
    if (!ud->tag || strcmp(ud->tag, "os.File") != 0) {
        luna_throw(vm, luna_fe(vm)->type_error_class, "%s() invalid file handle", fn);
    }
    return ud;
}
...
static Value file_flush(VM *vm, Value *args, int n) {
    ...
    if (fflush(file->fp) != 0) {
        luna_throw(vm, luna_fe(vm)->runtime_error_class, "flush() failed");
    }
    return make_null();
}
...
static Value file_write(VM *vm, Value *args, int n) {
    if (n < 2) {
        luna_throw(vm, luna_fe(vm)->argument_error_class, "write() requires a value argument");
    }
    ...
    free(text);
    if (written != len) {
        luna_throw(vm, luna_fe(vm)->runtime_error_class, "write() failed");
    }
...
static Value file_read_all(VM *vm, Value *args, int n) {
    ...
    if (fseek(file->fp, 0, SEEK_END) != 0) {
        luna_throw(vm, luna_fe(vm)->runtime_error_class, "read_all() failed");
    }
...
static Value os_open(VM *vm, Value *args, int n) {
    if (n < 2) {
        luna_throw(vm, luna_fe(vm)->argument_error_class, "os.open() requires path and mode");
    }
    ...
    if (!fp) {
        free(path);
        free(mode);
        luna_throw(vm, luna_fe(vm)->runtime_error_class, "os.open: failed to open file");
    }
...
static Value os_exists(VM *vm, Value *args, int n) {
    if (n < 1) {
        luna_throw(vm, luna_fe(vm)->argument_error_class, "os.exists() requires a path");
    }
...
static Value os_read_file(VM *vm, Value *args, int n) {
    if (n < 1) {
        luna_throw(vm, luna_fe(vm)->argument_error_class, "os.read_file() requires a path");
    }
    char *path = value_to_cstring(vm, args[0], "read_file", 1);
    FILE *fp = fopen(path, "rb");
    if (!fp) {
        free(path);
        luna_throw(vm, luna_fe(vm)->runtime_error_class, "os.read_file: failed to open file");
    }

    if (fseek(fp, 0, SEEK_END) != 0) {
        fclose(fp);
        free(path);
        luna_throw(vm, luna_fe(vm)->runtime_error_class, "read_file() failed");
    }
    long size = ftell(fp);
    if (size < 0) {
        fclose(fp);
...
static Value os_write_file(VM *vm, Value *args, int n) {
    if (n < 2) {
        luna_throw(vm, luna_fe(vm)->argument_error_class, "os.write_file() requires path and data");
    }
    ...
    if (!fp) {
        free(path);
        free(data);
        luna_throw(vm, luna_fe(vm)->runtime_error_class, "os.write_file: failed to open file");
    }
...
static Value os_append_file(VM *vm, Value *args, int n) {
    if (n < 2) {
        luna_throw(vm, luna_fe(vm)->argument_error_class, "os.append_file() requires path and data");
    }
    ...
    if (!fp) {
        free(path);
        free(data);
        luna_throw(vm, luna_fe(vm)->runtime_error_class, "os.append_file: failed to open file");
    }
    size_t len = strlen(data);
    size_t written = fwrite(data, 1, len, fp);
    int ok = fclose(fp);
...
static Value os_rename(VM *vm, Value *args, int n) {
    if (n != 2) {
        luna_throw(vm, luna_fe(vm)->argument_error_class, "os.rename() expects exactly 2 arguments");
    }
    require_string(vm, args[0], "os.rename", 1);
    require_string(vm, args[1], "os.rename", 2);
    if (rename(as_cstring(args[0]), as_cstring(args[1])) != 0) {
        luna_throw(vm, luna_fe(vm)->runtime_error_class, "os.rename(): %s", strerror(errno));
    }
    return make_null();
}
...
static Value os_remove(VM *vm, Value *args, int n) {
    if (n != 1) {
        luna_throw(vm, luna_fe(vm)->argument_error_class, "os.remove() expects exactly 1 argument");
    }
...
static Value os_stat(VM *vm, Value *args, int n) {
    if (n != 1) {
        luna_throw(vm, luna_fe(vm)->argument_error_class, "os.stat() expects exactly 1 argument");
    }
    ...
    struct _stat st;
    if (_stat(path, &st) != 0) {
        luna_throw(vm, luna_fe(vm)->runtime_error_class, "os.stat(): %s", strerror(errno));
    }
...
static Value os_execute(VM *vm, Value *args, int n) {
    if (n != 1) {
        luna_throw(vm, luna_fe(vm)->argument_error_class, "os.execute() expects exactly 1 argument");
    }
...
#else
    if (ret == -1) {
        luna_throw(vm, luna_fe(vm)->runtime_error_class, "os.execute(): %s", strerror(errno));
    }
    if (WIFEXITED(ret)) {
...
static Value os_args(VM *vm, Value *args_val, int n) {
    (void)args_val;
    if (n != 0) {
        luna_throw(vm, luna_fe(vm)->argument_error_class, "os.args() takes no arguments");
    }
...
static Value os_exit(VM *vm, Value *args, int n) {
    if (n != 1) {
        luna_throw(vm, luna_fe(vm)->argument_error_class, "os.exit() expects exactly 1 argument");
    }
    if (!IS_NUMBER(args[0])) {
        luna_throw(vm, luna_fe(vm)->type_error_class, "os.exit() argument must be numeric");
    }
...
static Value os_platform(VM *vm, Value *args, int n) {
    (void)args;
    if (n != 0) {
        luna_throw(vm, luna_fe(vm)->argument_error_class, "os.platform() takes no arguments");
    }
#ifdef _WIN32
...
static Value os_getpid(VM *vm, Value *args, int n) {
    (void)args;
    if (n != 0) {
        luna_throw(vm, luna_fe(vm)->argument_error_class, "os.getpid() takes no arguments");
    }
#ifdef _WIN32
...
static Value os_hostname(VM *vm, Value *args, int n) {
    (void)args;
    if (n != 0) {
        luna_throw(vm, luna_fe(vm)->argument_error_class, "os.hostname() takes no arguments");
    }
    char buf[256];
#ifdef _WIN32
    DWORD size = sizeof(buf);
    if (!GetComputerNameA(buf, &size)) {
        luna_throw(vm, luna_fe(vm)->runtime_error_class, "os.hostname(): system call failed");
    }
#else
    if (gethostname(buf, sizeof(buf)) != 0) {
        luna_throw(vm, luna_fe(vm)->runtime_error_class, "os.hostname(): %s", strerror(errno));
    }
#endif
    buf[sizeof(buf) - 1] = '\0';
    return make_obj((Object*)new_string(buf, (int)strlen(buf)));
...
static Value os_username(VM *vm, Value *args, int n) {
    (void)args;
    if (n != 0) {
        luna_throw(vm, luna_fe(vm)->argument_error_class, "os.username() takes no arguments");
    }
#ifdef _WIN32
    char buf[256];
    DWORD size = sizeof(buf);
    if (!GetUserNameA(buf, &size)) {
        luna_throw(vm, luna_fe(vm)->runtime_error_class, "os.username(): system call failed");
    }
    return make_obj((Object*)new_string(buf, (int)strlen(buf)));
#else
    struct passwd *pw = getpwuid(getuid());
    if (!pw || !pw->pw_name) {
        luna_throw(vm, luna_fe(vm)->runtime_error_class, "os.username(): unable to determine user");
    }
...
static Value os_tmpdir(VM *vm, Value *args, int n) {
    (void)args;
    if (n != 0) {
        luna_throw(vm, luna_fe(vm)->argument_error_class, "os.tmpdir() takes no arguments");
    }
#ifdef _WIN32
    char buf[MAX_PATH + 1];
    DWORD len = GetTempPathA(sizeof(buf), buf);
    if (len == 0 || len > sizeof(buf)) {
        luna_throw(vm, luna_fe(vm)->runtime_error_class, "os.tmpdir(): system call failed");
    }
...
static Value os_getenv(VM *vm, Value *args, int n) {
    if (n != 1) {
        luna_throw(vm, luna_fe(vm)->argument_error_class, "os.getenv() expects exactly 1 argument");
    }
...
static Value os_setenv(VM *vm, Value *args, int n) {
    if (n != 2) {
        luna_throw(vm, luna_fe(vm)->argument_error_class, "os.setenv() expects exactly 2 arguments");
    }
    ...
    if (_putenv_s(key, val) != 0) {
        luna_throw(vm, luna_fe(vm)->runtime_error_class, "os.setenv(): %s", strerror(errno));
    }
#else
...
static Value os_path_join(VM *vm, Value *args, int n) {
    if (n < 2) {
        luna_throw(vm, luna_fe(vm)->argument_error_class, "os.path_join() expects at least 2 arguments");
    }
    for (int i = 0; i < n; i++) {
    ...
    if (!buf) {
        luna_throw(vm, luna_fe(vm)->runtime_error_class, "os.path_join(): out of memory");
    }
