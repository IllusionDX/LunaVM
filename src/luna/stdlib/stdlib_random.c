...
static Value rng_int(VM *vm, Value *args, int n) {
    ...
    if (!state_list)
        luna_throw(vm, luna_fe(vm)->runtime_error_class, "Random instance corrupted: missing _state");
    ...
    if (n != 3) {
        luna_throw(vm, luna_fe(vm)->argument_error_class, "Random.int() expects 0 or 2 arguments (min, max)");
    }

    if (!IS_NUMBER(args[1]))
        luna_throw(vm, luna_fe(vm)->type_error_class, "Random.int(): min must be numeric");
    if (!IS_NUMBER(args[2]))
        luna_throw(vm, luna_fe(vm)->type_error_class, "Random.int(): max must be numeric");
...
static Value rng_float(VM *vm, Value *args, int n) {
    if (n < 1)
        luna_throw(vm, luna_fe(vm)->argument_error_class, "Random.float() called without instance");
    ...
    if (n >= 3) {
        if (!IS_NUMBER(args[1]))
            luna_throw(vm, luna_fe(vm)->type_error_class, "Random.float(): min must be numeric");
        if (!IS_NUMBER(args[2]))
            luna_throw(vm, luna_fe(vm)->type_error_class, "Random.float(): max must be numeric");
        double min = value_to_double(args[1]);
        double max = value_to_double(args[2]);
        return make_double(min + normalized * (max - min));
    }

    luna_throw(vm, luna_fe(vm)->argument_error_class, "Random.float() takes 0 or 2 arguments");
...
static Value rng_seed(VM *vm, Value *args, int n) {
    if (n < 2)
        luna_throw(vm, luna_fe(vm)->argument_error_class, "Random.seed() requires 1 argument");
    if (!IS_NUMBER(args[1]))
        luna_throw(vm, luna_fe(vm)->type_error_class, "Random.seed(): seed must be numeric");
...
static Value rng_call(VM *vm, Value *args, int n) {
    if (n != 1 && n != 3) {
        luna_throw(vm, luna_fe(vm)->argument_error_class, "Random() takes 0 or 2 arguments");
    }
    ...
    if (!IS_NUMBER(args[1]))
        luna_throw(vm, luna_fe(vm)->type_error_class, "Random(): first argument must be numeric");
    if (!IS_NUMBER(args[2]))
        luna_throw(vm, luna_fe(vm)->type_error_class, "Random(): second argument must be numeric");
...
static Value rng_choice(VM *vm, Value *args, int n) {
    if (n < 2)
        luna_throw(vm, luna_fe(vm)->argument_error_class, "choice() requires a sequence argument");
    ...
    Value seq = args[1];
    if (!IS_OBJ(seq))
        luna_throw(vm, luna_fe(vm)->type_error_class, "choice(): argument must be a list or string");
    ...
    if (obj->type->kind == OBJ_LIST) {
        ...
        if (len == 0)
            luna_throw(vm, luna_fe(vm)->value_error_class, "choice(): list is empty");
        uint64_t raw = rng_step(state_list);
    ...
    if (obj->type->kind == OBJ_STRING) {
        ...
        if (str->length == 0)
            luna_throw(vm, luna_fe(vm)->value_error_class, "choice(): string is empty");
        uint64_t raw = rng_step(state_list);
...
static Value rng_shuffle(VM *vm, Value *args, int n) {
    if (n < 2)
        luna_throw(vm, luna_fe(vm)->argument_error_class, "shuffle() requires a list argument");

    if (!IS_OBJ(args[1]) || AS_OBJ(args[1])->type->kind != OBJ_LIST)
        luna_throw(vm, luna_fe(vm)->type_error_class, "shuffle(): argument must be a list");
...
static Value random_WyRand(VM *vm, Value *args, int n) {
    ...
    if (n >= 1) {
        if (!IS_NUMBER(args[0]))
            luna_throw(vm, luna_fe(vm)->type_error_class, "WyRand(): seed must be numeric");
...
static Value random_Xoshiro256(VM *vm, Value *args, int n) {
    ...
    if (n >= 1) {
        if (!IS_NUMBER(args[0]))
            luna_throw(vm, luna_fe(vm)->type_error_class, "Xoshiro256(): seed must be numeric");
        seed = seed_from_arg(args[0]);
    }
...
static Value random_PCG(VM *vm, Value *args, int n) {
    ...
    if (n >= 1) {
        if (!IS_NUMBER(args[0]))
            luna_throw(vm, luna_fe(vm)->type_error_class, "PCG(): seed must be numeric");
        seed = seed_from_arg(args[0]);
    }
