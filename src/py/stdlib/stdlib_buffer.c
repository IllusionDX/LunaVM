...
static Value buffer_new(VM *vm, Value *args, int n) {
    if (n > 1) {
        luna_throw(vm, py_fe(vm)->argument_error_class, "buffer.new() expects at most 1 argument");
    }
    size_t capacity = 0;
    if (n == 1) {
...
static Value buffer_from_string(VM *vm, Value *args, int n) {
    if (n != 1) {
        luna_throw(vm, py_fe(vm)->argument_error_class, "buffer.from_string() expects exactly 1 argument");
    }
    if (!IS_STRING(args[0])) {
        luna_throw(vm, py_fe(vm)->type_error_class, "buffer.from_string() argument must be a string");
    }
...
void vm_register_buffer_module(VM *vm) {
    ...
    dict_set(py_fe(vm)->module_cache, make_obj((Object *)mod_key), mod_val);
}
