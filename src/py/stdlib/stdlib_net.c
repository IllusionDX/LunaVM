...
static Value socket_connect(VM *vm, Value *args, int n) {
    ...
    if (s->connected) {
        luna_throw(vm, py_fe(vm)->runtime_error_class, "connect(): socket already connected");
    }
...
static Value socket_accept(VM *vm, Value *args, int n) {
    ...
    if (!client_sock) {
        ...
        close(client_fd);
#endif
        luna_throw(vm, py_fe(vm)->runtime_error_class, "accept(): out of memory");
    }
...
static Value socket_recv(VM *vm, Value *args, int n) {
    ...
    if (!buf) {
        luna_throw(vm, py_fe(vm)->runtime_error_class, "recv(): out of memory");
    }
