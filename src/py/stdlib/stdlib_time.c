...
static inline double checked_number(VM *vm, Value v, const char *fn) {
    if (!IS_NUMBER(v)) {
        luna_throw(vm, py_fe(vm)->type_error_class, "%s() argument must be numeric", fn);
    }
...
static Value time_now(VM *vm, Value *args, int n) {
    (void)args;
    if (n != 0) {
        luna_throw(vm, py_fe(vm)->argument_error_class, "time.now() takes no arguments");
    }
...
static Value time_ticks_usec(VM *vm, Value *args, int n) {
    (void)args;
    if (n != 0) {
        luna_throw(vm, py_fe(vm)->argument_error_class, "time.ticks_usec() takes no arguments");
    }
...
static Value time_ticks_msec(VM *vm, Value *args, int n) {
    (void)args;
    if (n != 0) {
        luna_throw(vm, py_fe(vm)->argument_error_class, "time.ticks_msec() takes no arguments");
    }
...
static Value time_sleep(VM *vm, Value *args, int n) {
    if (n != 1) {
        luna_throw(vm, py_fe(vm)->argument_error_class, "time.sleep() expects exactly 1 argument");
    }
